"""
All prompt templates for the math synthesis framework.

This module contains the system prompts and user prompt templates
used by the various agents in the pipeline.
"""

from typing import Optional
from utils.llm_client import compose_prompt_examples
...
EVOLUTION_USER_PROMPT_TEMPLATE = """
The description of the original problem is:
{problem_description}
The solution steps are:
{solution_steps}
The answer is: {answer}
Now, begin your adaptation of this problem.
"""

SOLVABILITY_USER_PROMPT_TEMPLATE = """
The description of this problem is:
{problem_description}
The tentative solution steps are:
{solution_steps}
The answer is: {answer}
Now, you are to begin a detailed, step-by-step verification of this problem's solvability.
"""
...
{original_solution_steps}
The answer of the original problem is: {original_answer}
----------------------
The description of the new problem is:
{new_problem_description}
The tentative solution steps of the new problem are:
{new_solution_steps}
The answer of the new problem is: {new_answer}
---------------------
Now, begin your work. Adopting the persona of an experienced mathematics competitor and strictly adhering to the 5-point scoring rubric, evaluate the quality of the adaptation from the original problem to the new problem. Your core task is to determine if the new problem significantly elevates the 'burden of discovery' rather than merely increasing computational or procedural load. Produce your final score, status, and detailed reason.
"""


# =============================================================================
# Self-Refine Mode: Refinement User Prompt Templates
# =============================================================================

REFINE_SOLVABILITY_USER_PROMPT_TEMPLATE = """
## REFINEMENT TASK: Fix Solvability Issues

You previously evolved the following original problem, but the evolved problem **FAILED solvability verification**. Your task is to refine the evolved problem to fix the identified issues while maintaining or increasing its difficulty.
...
### Your Previous Evolved Problem (FAILED):
**Problem:** {failed_new_problem}

**Solution Steps:** {failed_new_solution_steps}

**Answer:** {failed_new_answer}

----------------------

### Verifier's Feedback (Why It Failed):
{verifier_feedback}

----------------------

### Your Task:
Based on the verifier's feedback above, you must now create a **refined version** of your evolved problem that:
1. **Fixes all solvability issues** identified in the feedback
2. **Maintains or increases** the conceptual difficulty compared to the original
3. **Ensures mathematical correctness** in every step of the solution
4. **Provides a valid, verifiable answer**

Think carefully about what went wrong and how to fix it. You may need to:
- Adjust constraints to avoid contradictions
- Fix mathematical errors in the solution
- Ensure the problem is well-defined and solvable
- Verify that the answer follows correctly from the solution steps

Now, begin your refinement work. Follow the same Thought-Code-Observation cycle and output format as before.
"""


REFINE_DIFFICULTY_USER_PROMPT_TEMPLATE = """
## REFINEMENT TASK: Increase Problem Difficulty

You previously evolved the following original problem, but the evolved problem **FAILED difficulty verification** - it was not considered sufficiently harder than the original. Your task is to refine the evolved problem to significantly increase its conceptual difficulty.
...
### Your Previous Evolved Problem (FAILED Difficulty Check):
**Problem:** {failed_new_problem}
...
### Difficulty Verifier's Feedback:
**Score:** {difficulty_score}/5
**Status:** FAIL
...
### Your Task:
Based on the verifier's feedback above, you must now create a **refined version** of your evolved problem that:
1. **Significantly increases the "burden of discovery"** - the difficulty should come from deeper conceptual insight, not more computation
2. **Resists standard solution templates** - the entry point should not be obvious
3. **Requires a genuine "Eureka" moment** - the solver should need to discover a non-obvious insight
4. **Maintains mathematical correctness** - the problem must still be solvable with a valid answer

Consider these strategies to increase difficulty:
- **Structural Obfuscation**: Hide a simple principle behind complex algebraic structures
- **Insight-Driven Construction**: Require a non-obvious auxiliary construction or transformation
- **Deep Pattern Discovery**: Force exploration and pattern recognition before the solution becomes clear
- **Cross-Domain Connection**: Require insights from multiple mathematical areas

Remember: We want problems that are hard because they require **conceptual breakthroughs**, not because they require **lengthy calculations**.

Now, begin your refinement work. Follow the same Thought-Code-Observation cycle and output format as before.
"""


REFINE_COMBINED_USER_PROMPT_TEMPLATE = """
## REFINEMENT TASK: Fix Both Solvability AND Difficulty Issues

You previously evolved the following original problem, but the evolved problem **FAILED verification**. Your task is to refine the evolved problem to fix ALL identified issues.
...
### Your Task:
Based on the verifier feedback above, you must now create a **refined version** of your evolved problem that:

1. **PRIORITY 1 - Mathematical Correctness**: The problem MUST be solvable with a valid, verifiable answer
   - Fix any logical errors, contradictions, or unsolvable conditions
   - Ensure every step in the solution follows correctly from the previous
   - Verify the final answer is correct and well-defined

2. **PRIORITY 2 - Conceptual Difficulty**: The problem should be significantly harder than the original
   - Increase the "burden of discovery" - require deeper conceptual insight
   - Resist standard solution templates - the entry point should not be obvious
   - Require a genuine "Eureka" moment to solve

**IMPORTANT**: You must satisfy BOTH requirements. A problem that is difficult but unsolvable is worthless. A problem that is solvable but trivially easy is also a failure.

Consider these strategies:
- **Fix first, then enhance**: Address any solvability issues before trying to increase difficulty
- **Structural Obfuscation**: Hide a simple principle behind complex algebraic structures
- **Cross-Domain Connection**: Require insights from multiple mathematical areas
- **Insight-Driven Construction**: Require a non-obvious auxiliary construction
...
# =============================================================================
# Planning Prompt (for strategic analysis)
# =============================================================================
...
# =============================================================================
# Solvability Verification Prompt
# =============================================================================
...
2.  **`Code:`** Execute your plan using Python and `sympy`.
...
1.  Your workflow must strictly follow the `Thought -> Code -> Observation` cycle. Each turn you take **MUST** consist of a `Thought` block followed by a `Code` block. Do not deviate from this structure.
2.  Use only variables that you have defined yourself or that were provided.
3.  Always use the right arguments for the tools.
4.  Focus each code block on a single, verifiable logical/mathematical task.
5.  You can perform multiple calculations on a variable within a single code block, as long as the final output is printed for verification.
6.  Call a tool only when needed. Do not re-run a calculation with the same parameters.
7.  Do not name any new variable with the same name as a tool.
8.  Do not use placeholder or example values.
9.  You can use imports from the following list: `{{authorized_imports}}`. Cleverly utilize Python code and its allowed libraries (e.g., sympy) as an analytical instrument to support your judgment.
10. The state persists between code executions.
11. Your final output must be a single call to the `final_answer` tool.
12. Be meticulous and objective. Your role is a strict verifier. Any discrepancy must be reported and result in a failure.
...
# =============================================================================
# Difficulty Verification Prompts
# =============================================================================
...
def get_evolve_prompt(
    ...
    base_dir: str = "math_demonstrations",
) -> str:
    """
    Get the evolution prompt, optionally with demonstrations.

    Args:
        with_demonstrations: Whether to include demonstrations
        demonstration_num: Number of demonstrations to include
        base_dir: Base directory for demonstration files

    Returns:
        The configured prompt template
    """
    if with_demonstrations:
        demonstration_str = compose_prompt_examples(
            example_num=demonstration_num,
            base_dir=base_dir,
        )
        return EVOLVE_PROMPT_TEMPLATE_WITH_DEMONSTRATIONS.format(
            demonstrations=demonstration_str
        )
    else:
        return EVOLVE_PROMPT_TEMPLATE
...
def get_difficulty_prompt(
    ...
    """
    Get the difficulty verification prompt, optionally with demonstrations.
    ...
    if with_demonstrations:
        ...
        return DIFFICULTY_PROMPT_TEMPLATE_WITH_DEMONSTRATIONS.replace(
            '{demonstrations}',
            demonstration_str
        )
    else:
        return DIFFICULTY_PROMPT_TEMPLATE
...
3. **Output Format**: Output your solution using the markdown format specified below. Include both section headers exactly as shown.

# Output Format
Your response must contain these two sections with exact headers:

## Solution Steps
(Provide numbered steps with detailed explanations and calculations)

## Answer
(Provide the final answer - can be a numerical value, expression in LaTeX, or "None" if unsolvable)

# Output Example
## Solution Steps
1. **Understand the problem**: We need to find all positive integers $n$ such that $n^2 + 2n + 1$ divides $n^4 + 4n^3 + 6n^2 + 4n + 1$.

2. **Simplify the divisor**: Notice that $n^2 + 2n + 1 = (n+1)^2$.

3. **Simplify the dividend**: Observe that $n^4 + 4n^3 + 6n^2 + 4n + 1 = (n+1)^4$ by the binomial expansion.

4. **Set up the divisibility condition**: We need $(n+1)^2 \mid (n+1)^4$, which simplifies to checking if $(n+1)^2 \mid (n+1)^4$.

5. **Analyze the quotient**: $\frac{(n+1)^4}{(n+1)^2} = (n+1)^2$, which is always an integer for any positive integer $n$.

6. **Conclude**: Since $(n+1)^2$ is always a positive integer when $n$ is a positive integer, the divisibility condition holds for all positive integers $n$.

## Answer
All positive integers $n$ satisfy the condition, i.e., $n \in \mathbb{Z}^+$
...
def format_solve_prompt(problem_description: str, legacy: bool = False) -> str:
    """
    Format the solve prompt with the given problem description.

    Args:
        problem_description: The math problem to solve
        legacy: If True, use the legacy JSON-based prompt format

    Returns:
        Formatted prompt
    """
    template = LEGACY_SOLVE_PROMPT if legacy else SOLVE_PROMPT
    return template.replace('{{problem}}', problem_description)


def format_check_solution_prompt(
    problem: str,
    solution_steps: str,
    final_answer: str,
) -> str:
    """
    Format the check solution prompt with the given inputs.

    Args:
        problem: The math problem description
        solution_steps: The solution steps
        final_answer: The final answer

    Returns:
        Formatted prompt
    """
    prompt = CHECK_SOLUTION_PROMPT
    prompt = prompt.replace('{{problem}}', problem)
    prompt = prompt.replace('{{solution_steps}}', solution_steps)
